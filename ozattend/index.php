        ...
        <ul id="mymyUL">
            ...
            if ($my_result->num_rows > 0) {
                ...
                while ($my_row = $my_result->fetch_assoc()) {
                    if ((strcasecmp(date("Y-m-d"), $my_row['eventDate']) == 0 && strcasecmp(date("H:i:s"), $my_row['startTime']) >= 0) || strcasecmp(date("Y-m-d"), $my_row['eventDate']) > 0) {
                        $goto = "show_att.php";
                    } else {
                        $goto = "edit.php";
                    }
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if (strcasecmp(date("Y-m-d"), $row['eventDate']) == 0 && strcasecmp(date("H:i:s"), $row['startTime']) >= 0 && strcasecmp(date("H:i:s"), $row['endTime']) <= 0) {
            $activeStatus = "&nbsp&nbsp&nbsp<wbr> <blink>LIVE</blink>";
        } else {
            $activeStatus = "";
        }

        if ((strcasecmp(date("Y-m-d"), $row['eventDate']) == 0 && strcasecmp(date("H:i:s"), $row['endTime']) <= 0) || strcasecmp(date("Y-m-d"), $row['eventDate']) < 0) {
            echo
